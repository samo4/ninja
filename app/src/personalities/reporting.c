...
ZBUS_CHAN_DEFINE(timer_chan, struct timer_msg, NULL, NULL,
...
static void timer_arm(uint32_t delay_sec) {
	...
	err = k_work_reschedule(&sample_timer_work, K_SECONDS(delay_sec));
	if (err < 0) {
...
static void fire_sample(struct reporting_state *state) {
	...
	err = zbus_chan_pub(&led_chan, &led, PUB_TIMEOUT);
	if (err) {
...
static const struct smf_state states[] = {
	[STATE_SAMPLING] =
		SMF_CREATE_STATE(sampling_entry, NULL, NULL, NULL, NULL),
	[STATE_WAITING] =
		SMF_CREATE_STATE(NULL, waiting_run, NULL, NULL, NULL),
	[STATE_REBOOTING] =
		SMF_CREATE_STATE(rebooting_entry, NULL, NULL, NULL, NULL),
};
...
static enum smf_state_result waiting_run(void *o) {
	...
	if (state->chan == &timer_chan) {
		const struct timer_msg *msg =
			(const struct timer_msg *)state->msg_buf;
