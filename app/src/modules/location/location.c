...
BUILD_ASSERT(CONFIG_APP_LOCATION_WATCHDOG_TIMEOUT_SECONDS > CONFIG_APP_LOCATION_MSG_PROCESSING_TIMEOUT_SECONDS,
...
ZBUS_CHAN_DEFINE(location_chan, struct location_msg, NULL, NULL, ZBUS_OBSERVERS_EMPTY, ZBUS_MSG_INIT(0));
...
ZBUS_CHAN_DEFINE(priv_location_chan, struct priv_location_msg, NULL, NULL, ZBUS_OBSERVERS(location), ZBUS_MSG_INIT(0));
...
enum location_module_state {
    STATE_WAITING_FOR_CFUN, // Waiting for modem functional mode to be set
    STATE_RUNNING,
...
static const struct smf_state states[] = {
    [STATE_WAITING_FOR_CFUN] = SMF_CREATE_STATE(NULL, state_waiting_for_cfun_run, NULL, NULL, NULL),
    [STATE_RUNNING] = SMF_CREATE_STATE(state_running_entry, NULL, NULL, NULL, &states[STATE_LOCATION_SEARCH_INACTIVE]),
    [STATE_LOCATION_SEARCH_INACTIVE] = SMF_CREATE_STATE(
        state_location_search_inactive_entry, state_location_search_inactive_run, NULL, &states[STATE_RUNNING], NULL),
    [STATE_LOCATION_SEARCH_ACTIVE] = SMF_CREATE_STATE(
...
static void cloud_request_send(const struct location_data_cloud *cloud_request) {
    ...
    int err = location_cloud_request_data_copy(&location_msg.cloud_request, cloud_request);
    if (err) {
...
static void agnss_request_send(const struct nrf_modem_gnss_agnss_data_frame *agnss_request) {
    struct location_msg location_msg = {.type = LOCATION_AGNSS_REQUEST, .agnss_request = *agnss_request};
...
static enum smf_state_result state_waiting_for_cfun_run(void *obj) {
    ...
    if (state_object->chan == &priv_location_chan) {
        const struct priv_location_msg *msg = (const struct priv_location_msg *)state_object->msg_buf;
...
static void state_running_entry(void *obj) {
    ...
    const struct location_msg ready_msg = {.type = LOCATION_MODULE_READY};
...
static enum smf_state_result state_location_search_active_run(void *obj) {
    ...
    if (state_object->chan == &location_chan) {
        ...
        if (location_msg->type == LOCATION_SEARCH_TRIGGER || location_msg->type == LOCATION_GNSS_SEARCH_TRIGGER) {
            ...
            smf_set_state(SMF_CTX(state_object), &states[STATE_LOCATION_SEARCH_INACTIVE]);
            return SMF_EVENT_HANDLED;
        }
    }
...
static void location_print_data_details(enum location_method method, const struct location_data_details *details) {
...
static void location_event_handler(const struct location_event_data *event_data) {
    switch (event_data->id) {
        case LOCATION_EVT_LOCATION:
            LOG_DBG("Got location: lat: %f, lon: %f, acc: %f, method: %s", (double)event_data->location.latitude,
        ...
        case LOCATION_EVT_ERROR:
            ...
            LOG_WRN("Used method: %s (%d)", location_method_str(event_data->method), event_data->method);
        ...
        case LOCATION_EVT_FALLBACK:
            ...
            LOG_DBG("Failed method: %s (%d)", location_method_str(event_data->method), event_data->method);
            LOG_DBG("New method: %s (%d)", location_method_str(event_data->fallback.next_method),
                    event_data->fallback.next_method);
            LOG_DBG("Cause: %s", (event_data->fallback.cause == LOCATION_EVT_TIMEOUT) ? "timeout"
