...
ZBUS_CHAN_DEFINE(led_chan, struct led_msg, NULL, NULL, ZBUS_OBSERVERS_EMPTY, ZBUS_MSG_INIT(0));
...
static void blink_timer_handler(struct k_work *work) {
    ...
    uint32_t next_delay =
        led_state.is_on ? led_state.current_state.duration_on_msec : led_state.current_state.duration_off_msec;
